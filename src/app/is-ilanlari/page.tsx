  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 pb-24 md:pb-12">
        <div className="rounded-2xl overflow-hidden border border-[#e0e8f5]">
          <div className="bg-[#0000FF] px-8 py-16 text-center">
            ...
            <a href="mailto:info@otrsocial.com?subject=İş İlanı Erken Erişim" className="inline-block bg-white text-[#0a0a0a] text-[13px] font-medium px-6 py-2.5 rounded-xl hover:bg-ink-100 transition-colors">Erken erişim için bize yazın →</a>
            <p className="text-[12px] text-white/25 mt-5 italic">Yanıt süresi: 24 saat. Spam süresi: sıfır.</p>
          </div>
          <div className="bg-white px-8 py-6 grid grid-cols-3 gap-4 border-t border-[#e0e8f5]">
            {[
              { sayi: '4.2K+', label: 'aktif kullanıcı' },
              { sayi: '200+', label: 'şirket profili' },
              { sayi: '%100', label: 'beyaz yaka kitle' },
            ].map(item => (
              <div key={item.label} className="p-4 border border-[#e0e8f5] rounded-xl">
                <p className="text-[22px] font-semibold text-[#0a0a0a] mb-1">{item.sayi}</p>
                <p className="text-[12px] text-ink-400">{item.label}</p>
              </div>
            ))}
          </div>
          <div className="bg-[#f0f6ff] px-8 py-6 border-t border-[#e0e8f5]">
            <p className="text-[12px] font-medium text-[#0a0a0a] mb-4">Sponsorlu ilan nasıl çalışır?</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div key={item.n} className="flex items-start gap-3">
                  <div className="w-7 h-7 rounded-lg bg-[#0000FF] flex items-center justify-center shrink-0">
                    <span className="text-white text-[11px] font-medium">{item.n}</span>
                  </div>
                  <div>
                    <p className="text-[12px] font-medium text-[#0a0a0a] mb-0.5">{item.baslik}</p>
