...
export default function MaasClient() {
  ...
  const sektorLabels = SEKTORLER.filter(sk => filtered.some(s => s.sektor === sk))
    ...
    setTimeout(() => {
      ...
      if (barRef.current) {
        barChart.current = new Chart(barRef.current, {
          type: 'bar',
          data: { labels: unvanSirasi, datasets: [{ data: unvanData, backgroundColor: barColor, borderRadius: 4 }] },
          options: {
            responsive: true, maintainAspectRatio: false,
            plugins: { legend: { display: false } },
            scales: {
              x: { ticks: { color: textColor, font: { size: 11 }, autoSkip: false, maxRotation: 0 }, grid: { display: false }, border: { display: false } },
              y: { ticks: { color: textColor, font: { size: 11 }, callback: (v: number) => v === 0 ? '' : (v / 1000) + 'K TL' }, grid: { color: gridColor }, border: { display: false } }
            }
          }
        })
      }
      ...
      if (hbarRef.current && sektorLabels.length > 0) {
        hbarChart.current = new Chart(hbarRef.current, {
          type: 'bar',
          data: { labels: sektorLabels, datasets: [{ data: sektorData, backgroundColor: barColor, borderRadius: 4 }] },
          options: {
            indexAxis: 'y',
            responsive: true, maintainAspectRatio: false,
            plugins: { legend: { display: false } },
            scales: {
              x: { ticks: { color: textColor, font: { size: 11 }, callback: (v: number) => v === 0 ? '' : (v / 1000) + 'K' }, grid: { color: gridColor }, border: { display: false } },
              y: { ticks: { color: textColor, font: { size: 12 } }, grid: { display: false }, border: { display: false } }
            }
          }
        })
      }
  ...
  const handleSubmit = async () => {
    if (!user) { setError('Maaş eklemek için giriş yapmanız gerekiyor'); return }
    ...
    const { error: err } = await supabase.from('salary_data').insert({
      user_id: user.id, sektor, unvan, sehir,
      maas: parseInt(maas),
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 pb-24 md:pb-8">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-[13px] text-ink-400 mt-0.5">Çalışanların gönüllü olarak paylaştığı anonim maaş verileri</p>
          </div>
          <button onClick={handleAddClick} className="text-[13px] font-medium text-white px-4 py-2 rounded-lg bg-ink-900 hover:bg-ink-700 transition-colors">
            + Maaşımı ekle
          </button>
        </div>
