      ...
      <div className="max-w-5xl mx-auto px-4 py-12 pb-24 md:pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-12">
          ...
          <div className="bg-white rounded-xl p-6 relative" style={{border: '2px solid #1a1a1a'}}>
            ...
            <div className="space-y-2.5 mb-8">
              ...
              <div className="flex items-center gap-2 text-[13px] text-ink-400"><span>✗</span>İş ilanı</div>
            </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { title: 'Standart ilan', desc: '30 gün · şirket profilinde', price: '1.990 ₺', subject: 'Standart İlan', primary: false },
              { title: 'Sponsored ilan', desc: '30 gün · profil + feed'de görünür', price: '4.900 ₺', subject: 'Sponsored İlan', primary: true },
              { title: '5'li standart paket', desc: '5 ilan · 30 gün her biri · %21 tasarruf', price: '7.900 ₺', subject: '5li Standart Paket', primary: false },
              { title: '5'li sponsored paket', desc: '5 ilan · 30 gün · feed'de · %27 tasarruf', price: '17.900 ₺', subject: '5li Sponsored Paket', primary: false },
            ].map(pkg => (
              <div key={pkg.title} className="bg-white rounded-xl p-6" style={{border: pkg.primary ? '2px solid #1a1a1a' : '0.5px solid var(--color-border)'}}>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="text-[15px] font-semibold text-ink-900">{pkg.title}</h3>
                    <p className="text-[13px] text-ink-400 mt-0.5">{pkg.desc}</p>
                  </div>
                  <p className="text-[20px] font-semibold text-ink-900">{pkg.price}</p>
                </div>
                <a href={`mailto:iletisim@otrsocial.com?subject=${pkg.subject}`} className={`block text-center text-[13px] font-medium py-2 rounded-xl transition-colors ${pkg.primary ? 'text-white bg-ink-900 hover:bg-ink-700' : 'text-ink-900 border border-ink-200 hover:bg-ink-50'}`}>
                  {pkg.title.includes('paket') ? 'Paketi al' : 'İlan ver'}
                </a>
              </div>
            ))}
          </div>
